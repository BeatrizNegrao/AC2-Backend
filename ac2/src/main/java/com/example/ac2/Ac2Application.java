...
public class Ac2Application {
	...
	public CommandLineRunner init(@Autowired ProjetoRepository projetoRepository,
		...
		return args -> {
			...
			// criar projetos
			Projeto p1 = Projeto.builder().descricao("Sistema Interno").dataInicio(LocalDate.now())
					.dataFim(LocalDate.now().plusDays(30)).build();

			Projeto p2 = Projeto.builder().descricao("Portal Clientes").dataInicio(LocalDate.now().plusDays(10))
					.dataFim(LocalDate.now().plusDays(60)).build();
